...
class ShMatrix:
    ...
    def __init__(self, matrix:list):
        # сразу проверяем введенную матрицу
        # вот тут кстати непонятно, как определить какая длина строки матрица является правильной?
        # Например есть вот такие входные данные (изображу их построчно для удобства):
        # [1, 2, 3]     - эта длина верная?
        # [4, 5]        - может быть эта?
        # [6, 7, 8, 9]  - или эта длина?
        # так как в задании нет четкого определения как поступать в этом случае, а правила сложения матриц
        # требуют четкого и явного определения матрицы я решил, для того, чтобы исключить вариации
        # за основу длины матрицы я буду брать длину 0-го элемента
        ...
        for index, items in enumerate(matrix, 1):
            ...
            for item in items:
                if not isinstance(item, int):
                    raise Exception('Неверная инициализация матрицы! Все значения вложенных списков должны быть типа "int"')
        self.matrix = matrix
    ...
    def __add__(self, other):
        ...
        return ShMatrix(res)
# мне пока непривычно внезапно заканчивающийся класс, поэтому я для себя делаю комментарий что класс заканчивается здесь
# end class ShMatrix
